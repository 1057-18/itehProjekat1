<?php 
	...
	if(isset($_GET['akcija']) && $_GET['akcija']=="prodatBrojProizvodaPoDanima") {
		$sql="SELECT date(o.time_ordered) as date, count(ol.id) as num FROM orders_list as ol JOIN orders as o ON ol.order_id = o.id GROUP BY date(o.time_ordered)";
		if($q=$mysqli->query($sql)) {
			$niz = array();
			while($red = $q->fetch_object()) {
				$niz[] = $red;
